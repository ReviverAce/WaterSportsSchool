...
public class AddKlientFrame extends JFrame {
	 ...
	 * @param rectangle 
	 */
	public AddKlientFrame(int width, int height, int x1,int x2,int x3,int x4) {
	...
	private void initComponents(int width, int height, int x1,int x2,int x3,int x4) {
		...
		JLabel lblSystemZarzdzaniaSzko = new JLabel("Wprowadz dane dla nowego klienta",SwingConstants.CENTER);
		...
		JLabel lblStawka = new JLabel("Ile zapłacono:");
		...
		btnZatwierdz.addActionListener((ActionListener) new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					flag = false;
					JOptionPane.showMessageDialog(null, "Kwota zawiera niedozwolone znaki");
				}

				if (flag) {
					Klient.dodajOsoba(textField.getText(), textField_1.getText(), textField_2.getText(),
