...
public class AddInstruktorFrame extends JFrame {
	 ...
	 * @param rectangle
	 */
	public AddInstruktorFrame(int width, int height, int x1, int x2, int x3, int x4) {
	...
	private void initComponents(int width, int height, int x1, int x2, int x3, int x4) {
		...
		JLabel lblSystemZarzdzaniaSzko = new JLabel("Wprowadz dane dla nowego instruktora", SwingConstants.CENTER);
		...
		JLabel lblStopie = new JLabel("Stopień:");
		...
		comboBox.setModel(new DefaultComboBoxModel(Stopien.values()));
		...
		btnZatwierdz.addActionListener((ActionListener) new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					flag = false;
					JOptionPane.showMessageDialog(null, "Stawka zawiera niedozwolone znaki lub jest mniejsza od zera");
				}

				Stopien type;
				if (comboBox.getSelectedIndex() == 0) {
					type = Stopien.PODSTAWOWY;
				} else if (comboBox.getSelectedIndex() == 1) {
					type = Stopien.SREDNIOZAAWANSOWANY;
				} else {
					type = Stopien.ZAAWANSOWANY;
				}

				if (flag) {
					Instruktor.dodajOsoba(textField.getText(), textField_1.getText(), textField_2.getText(),
							Integer.parseInt(textField_3.getText()), type);
					JOptionPane.showMessageDialog(null, "Dodano instruktora");
