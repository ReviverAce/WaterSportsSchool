...
public class AddLatawiecFrame extends JFrame {
	 ...
	 * @param rectangle 
	 */
	public AddLatawiecFrame(int width, int height, int x1,int x2,int x3,int x4) {
	...
	private void initComponents(int width, int height, int x1,int x2,int x3,int x4) {
		...
		JLabel lblSystemZarzdzaniaSzko = new JLabel("Wprowadz dane dla nowego latawca",SwingConstants.CENTER);
		...
		btnZatwierdz.addActionListener((ActionListener) new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					flag = false;
					JOptionPane.showMessageDialog(null, "Rozmiar zawiera niedozwolone znaki lub jest mniejszy od zera");
				}
				...
				if (flag) {
					Latawiec.dodajSprzet(textField.getText(), Stan.WOLNY, Integer.parseInt(textField_1.getText()), textField_2.getText(), Integer.parseInt(textField_3.getText()), textField_4.getText());
					JOptionPane.showMessageDialog(null, "Dodano latawiec");
